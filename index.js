...
// server:  https://forms-handle-server.vercel.app/
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    const formActive = answers[1]?.c?.[0]?.t === "active"; // Checkbox or toggle for form active
    ...
    const apiKey = "lastkey123"; // Replace with your 123FormBuilder API key
    ...
    res
      .status(200)
      .json({ message: "Form received and sent to 123FormBuilder", formData });
  } catch (error) {
...
// app.post("/webhook", async (req, res) => {
//   try {
//     console.log("Received Webhooks Data:", JSON.stringify(req.body, null, 2));
//     console.log(
//       "Received Webhooks Data:",
//       req.body?.answer?.answers[1].c[0]?.t
//     );
...
//     const formActive = answers[1]?.c?.[0]?.t === "active"; // Assuming 'c' contains an array and the first item is "active"
